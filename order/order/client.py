...
from typing import Any, Awaitable, Callable, Coroutine, Optional, Tuple, Union
        ...
        async def wrap(order: Order) -> Optional[Coroutine[Any, Any, httpx.Response]]:
            ...
            for i in range(1, self.retry+1):
                ...
                #   Nginx close connection initiative(keepalive_requests)
                    ...
                    print(
                        f'Failed to send order: {order.oid}, with error {e}, retries: {i}')
                    try_reconnect = True
                    if i == self.retry:
                        raise
